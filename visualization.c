...
int main(int argc, char** argv){
	...
	while(!feof(f)){
			...
			if (data[2] == 0x10) {
				...
				if (maf != 0) { // Ignore 0 values
					if(prev_maf != maf) {
						...
						efc = ((maf / 14.6) * 44.4) * time_passed; //engine fuel consumption in kJ. 44.4kJ/g of gasoline
						print_joules = 1;
					}
		...
		if (id == 0x07EC) {
			...
			if (soc != 0) { // Ignore 0 values
				if (print_joules == 1) {
					float soc_diff = soc - prev_soc;
					bec = (6500 * soc_diff) * 288 * 3.6 * 0.001; //mAh * voltage * 3.6 = Joules of energy
					prev_soc = soc;
					print_joules = 0;
					printf("%f, %.2f\n", time_offset, efc + bec);
					//printf("%.2f %%, %.2fJ\n", soc, efc + bec);
